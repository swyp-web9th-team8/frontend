...
export default function Nav() {
  ...
  return (
    <div className="shadow-[0px_4px_24px_0px_rgba(170,170,170,0.30)]justify-center fixed bottom-0 -ml-5 flex w-full max-w-[500px] items-end bg-white pt-2.5 pb-5">
      {NAV_ITEMS.map((item, index) => {
        ...
        return (
          <Link
            key={index}
            href={item.href}
            className={`flex flex-1 flex-col items-center justify-center gap-0.5 ${isActive ? NAV_ITEM_STYLE.active.icon : NAV_ITEM_STYLE.default.icon} text-gsans-medium`}
          >
            {item.icon}
            <div
